...
import util.ChunkedOutputStream;
import util.StringUtil;

/**
 * A class for using Google's Duplex Speech API. Allows for continuous
 * recognition. Requires an API-Key. A duplex API opens two connections. One to
 * an upstream and one to a downstream. The system allows for continuous
 * chunking on both up and downstream. This, in turn, allows for Google to
 * return data as data is sent to it. For this reason, this class uses
 * listeners.
 *
...
public class GSpeechDuplex {
     ...
     *
     * @param flacFile The file you wish to upload. NOTE: Segment the file if
     ...
     */
    public void recognize(File flacFile, int sampleRate, simplePanel gui) throws IOException {
        this.gui=gui;
        recognize(mapFileIn(flacFile), sampleRate, gui);
    }

    /**
     * Send a byte[] to the URL with a specified sampleRate. NOTE: The byte[]
     * should contain no more than 15 seconds of audio. Chunking is not fully
     ...
     */
    public void recognize(byte[] data, int sampleRate, simplePanel gui) {
        this.gui=gui;
        if (data.length >= MAX_SIZE) {//Temporary Chunking. Does not allow for Google to gather context.
            ...
            for (byte[] array : dataArray) {
                recognize(array, sampleRate, gui);
            }
        }
        ...
        final String API_DOWN_URL = GOOGLE_DUPLEX_SPEECH_BASE + "down?key="+API_KEY+"&maxresults=1&pair=" + PAIR;
    ...
    /**
     * This code opens a new Thread that connects to the downstream URL. Due to
     * threading, the best way to handle this is through the use of listeners.
     *
     ...
     * @throws LineUnavailableException If cannot open or stream the
     * TargetDataLine.
     */
     ...
     * @param mtl The TargetDataLine you want to post data from. <b>Note should
     * be open</b>
    ...
    /**
     * Opens a chunked HTTPS post connection and returns a Scanner with incoming
     * data from Google Server Used for to get UPStream Chunked HTTPS ensures
     * unlimited file size.
     *
     ...
     * @return A Scanner to access the server response. (Probably will never be
     * used)
     */
    private Scanner openHttpsPostConnection(String urlStr, byte[][] data, int sampleRate) {
        ...
        try {
            ...
            try {
                ...
                // supply bytes to the urlConn Stream at a rate that approaches
                // the bitrate ( =30K per sec. in this instance ).
                gui.getjLabel1().setText("Working...");
                ...
                out.write(FINAL_CHUNK);
                System.out.println("IO WRITE DONE");
                gui.getjLabel1().setText("Translating...");
				// do you need the trailer?
                // NOW you can look at the status.
                resCode = httpConn.getResponseCode();
                if (resCode / 100 != 2) {
